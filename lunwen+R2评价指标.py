...
class CSVDataset(Dataset):
    def __init__(self, csv_file):
        ...
        self.features = data.loc[:, "x1":"x95"].values.astype(np.float32)
...
def train_model(model, train_loader, optimizer, criterion, device, epoch, total_epochs):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        outputs = model(features).squeeze()  # 修改为传递 features，而不是 x_numer
        ...
        # 每 50 个 batch 输出一次训练情况
        if batch_idx % 50 == 0:
            print(f"Epoch [{epoch+1}/{total_epochs}], Batch [{batch_idx+1}/{len(train_loader)}], Loss: {loss.item():.4f}")
...
def evaluate_model(model, test_loader, device):
    ...
    mae = mean_absolute_error(true_values, predictions)  # 计算 MAE

    return rmse, r2, dm_stat, mae, predictions, true_values
...
# 主函数
def main(train_file, test_file, epochs=15, batch_size=64, lr=1e-4):
    ...
    for epoch in range(epochs):
        train_loss = train_model(model, train_loader, optimizer, criterion, device, epoch, epochs)
        print(f"Epoch {epoch+1}/{epochs} - Training Loss: {train_loss:.4f}")

        # 每个 epoch 后评估一次模型
        rmse, r2, dm_stat, mae, predictions, true_values = evaluate_model(model, test_loader, device)
        print(
            f"Epoch {epoch + 1}/{epochs} - Evaluation - RMSE: {rmse:.4f}, R²: {r2:.4f}, DM: {dm_stat:.4f}, MAE: {mae:.4f}")
    ...
    # 在 main 函数的训练结束后添加以下代码
    torch.save(model.state_dict(), 'ft_transformer_licha    _model.pth')  # 保存模型权重
    print("Model saved to 'ft_transformer_model.pth'")

if __name__ == "__main__":
    train_file = "beifen——shujuji/train_split.csv"  # 训练集文件路径
    test_file = "beifen——shujuji/test_split.csv"    # 测试集文件路径
